...
from .config import CONFIG_FILE, load_config, save_to_history, save_config, DefaultConfig
from .terminal_utils import execute_command
from .core import get_ai_response, parse_command_blocks
...
HELP_TEXT = dedent("""
    ...
    - `!config` - Edit the configuration file (config.yaml)
    - `!model` - Show available models and current model
    - `!exit` or `Ctrl+C` - Exit the program
""")
...
def _handle_config(console: Console, config: DefaultConfig) -> DefaultConfig:
    ...
    # Try to reload the config after editing
    try:
        new_config = load_config()
        console.print(f"Config reloaded. Using model: [bold]{new_config['API']['model']}[/bold]")
        return new_config
    except Exception as e:
        console.print(f"[bold red]Error reloading config: {e}. Please check config.yaml or run --setup.[/bold red]")
        return config
...
def _handle_model(console: Console, config: DefaultConfig) -> None:
    ...
    change: str = input("\nChange model? (Enter model name or press Enter to keep current): ").strip()
    if change:
        if change not in AVAILABLE_MODELS:
            console.print(f"[bold red]Model '{change}' is not in the list of available models.[/bold red]")
            return
        ...
        try:
            ...
            console.print(f"[bold red]Error saving model change to config: {e}[/bold red]")
...
def _handle_exec(console: Console, cmd: str, allow_execution: bool) -> None:
    ...
    if not allow_execution:
        console.print("[bold red]Command execution is disabled.[/bold red] Enable it in config.yaml.")
        return
...
def _handle_command_blocks(console: Console, command_blocks: list[str], allow_execution: bool) -> None:
    """
    ...
    while True:
        block_num = input("\nEnter block number to execute (or press Enter to skip): ").strip()
        if not block_num:
            return
        if block_num.isdigit() and 1 <= int(block_num) <= len(command_blocks):
            ...
            console.print(f"[bold red]Invalid block number. Please enter a number between 1 and {len(command_blocks)}, or press Enter to skip.[/bold red]")
...
def interactive_mode(config: DefaultConfig, console: Console) -> None:
    ...
    def _main_loop(config: DefaultConfig) -> None:
        ...
        while True:
            ...
            try:
                response: str = get_ai_response(query, config)
            except Exception as e:
    ...
    # Welcome banner
    console.print("[bold green]AI Terminal Assistant - Interactive Mode[/bold green] ([bold]Ctrl+C[/bold] to exit)")
    console.print("Type [bold]!help[/bold] for special commands")
